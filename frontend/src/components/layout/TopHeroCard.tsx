...
export function TopHeroCard({ title, subtitle, stats }: Props) {
  const navigate = useNavigate();

  return (
    <div className="brand-panel-pattern relative overflow-hidden text-white rounded-(--radius-card) px-6 py-5 flex items-start justify-between gap-4 shadow-[0_18px_40px_rgba(2,88,100,0.18)]">
      ...
      <div className="relative">
        <p className="text-xs text-white/70">{subtitle ?? "Inventario en tiempo real"}</p>
        <h1 className="mt-1 text-3xl font-semibold tracking-[-0.02em]">{title}</h1>
        {stats && (
          <div className="flex gap-6 mt-4">
            {stats.map((s) => (
              <div key={s.label}>
                <p className="text-white/55 text-[11px] uppercase tracking-wide">{s.label}</p>
      ...
      <div className="relative flex gap-2 shrink-0 pt-5">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-white/14 hover:bg-white/22 text-white text-sm font-medium px-4 py-2 rounded-(--radius-btn) transition-colors border border-white/20"
        >
